...
if (!checkRequiredFiles(apps.requiredFiles)) {
  process.exit(1);
}

// The config is async as we need to read each dir under /apps to get each entry point for all the apps
getConfig().then(confRes => {
  let config = confRes.config;
  let appsRoots = confRes.appsDirList;

  // Make sure all sass files for each app are compiled
    console.log("reached")
  apps.buildSass(appsRoots).then(sassRes =>{
    console.log(chalk.green(sassRes));
    // Rread the current file sizes in build directory.
    // This lets us display how much they changed later.
    measureFileSizesBeforeBuild(appPaths.appBuild)
    ...
    .then(
      ({ stats, previousFileSizes, warnings }) => {
        ...
        const appPackage = require(appPaths.packageJson);
        const publicUrl = appPaths.publicUrl;
        const publicPath = config.output.publicPath;
        const buildFolder = path.relative(process.cwd(), appPaths.appBuild);
